...
describe('EventBus', () => {
  ...
  it('continues executing remaining handlers when one throws', async () => {
    ...
    bus.on('conversation.assigned', async () => { order.push(1); });
    bus.on('conversation.assigned', async () => { throw new Error('handler 2 failed'); });
    bus.on('conversation.assigned', async () => { order.push(3); });
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      'Event handler error for conversation.assigned:',
      expect.any(Error),
    );
